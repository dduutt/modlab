# Task 2 Brief: DataGrid 2-Register Cell Merging & 32-bit Display/Editing (`DataGrid.vue`)
...
## Instructions
1. Update `src/components/DataGrid.vue` to accept `byteOrder: string` prop and handle 32-bit data types (`Float32`, `Int32`, `UInt32`).
2. Pass `byteOrder` from `activeTab.config.byteOrder` in `src/App.vue`.
3. Support `update-cell-pair` emit in `DataGrid.vue` when editing 32-bit registers, and handle in `App.vue` to update both 16-bit words.
...
Write report to `file:///F:/pro/modlab/docs/superpowers/plans/task-ind-2-report.md`.
